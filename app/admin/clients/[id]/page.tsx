...
export default async function ViewClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 4. Fetch all chemicals (for dropdown assigning)
  ...
  // 9. Fetch Internal Notes
  const { data: internalNotesData } = await adminSupabase
    .from('internal_notes')
    .select('*, users!internal_notes_author_id_fkey(email)')
    .eq('client_id', id)
    .order('created_at', { ascending: false });

  const internalNotes = (internalNotesData || []).map((note: any) => ({
    id: note.id,
    note: note.note,
