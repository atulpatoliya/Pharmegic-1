...
import {
  convertReachDocxUrlToPdf,
  resolveReachCertificateDownloadFile,
} from '@/lib/reach-certificate-pdf';
import {
...
function attachmentResponse(buffer: Buffer, fileName: string) {
...
/** Server-side DOCX→PDF for RC download when preview uses Office Online embed. */
export async function GET(request: NextRequest) {
  ...
  if (docxUrl) {
    try {
      const file = await convertReachDocxUrlToPdf(docxUrl, fileName);
      return attachmentResponse(file.buffer, file.fileName);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Certificate PDF conversion failed.';
  ...
  return NextResponse.json(
    { error: 'docxUrl, certificateId, or clientId and chemicalId are required.' },
    { status: 400 }
  );
}
