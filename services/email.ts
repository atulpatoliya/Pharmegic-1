...
export async function sendCertificateEmail({
  to, cc, subject, certificateNumber, companyName, chemicalName, pdfBuffer, pdfFileName, smtpConfig,
}: SendCertificateEmailOptions) {
  const { transporter, from } = buildTransporter(smtpConfig);

  const html = getCertificateEmailHtml(companyName, chemicalName, certificateNumber);
